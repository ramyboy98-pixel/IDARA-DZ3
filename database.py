...
def seed_default_service_links():
    """
    إضافة روابط الخدمات الافتراضية مرة واحدة فقط.
    لا يكرر الرابط إذا كان موجودا بنفس المصلحة ونفس العنوان ونفس الرابط.
    """
    conn = connect_db()
    try:
        cursor = conn.cursor()
        ...
        for service_key, title, url in DEFAULT_SERVICE_LINKS:
            cursor.execute("""
                SELECT id
                FROM service_links
                WHERE service_key = ? AND title = ? AND url = ?
                LIMIT 1
            """, (service_key, title, url))
