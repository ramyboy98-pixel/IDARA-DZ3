...
class IdaraDZApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        self.search_entry.pack(fill="x", pady=(35, 20))
    ...
    def show_home(self):
        ...
        self.create_card(cards_frame, "📄", "وثائق", "استخراج الوثائق الإدارية", self.open_documents, 0)
        self.create_card(cards_frame, "🌐", "خدمات إلكترونية", "روابط وخدمات رسمية", self.open_services, 1)
        self.create_card(cards_frame, "🗂️", "أرشيف", "حفظ وبحث وإعادة طباعة", self.open_archive, 2)

    def create_card(self, parent, icon, title, subtitle, command, column):
        card = ctk.CTkButton(
            parent,
            text=f"{icon}\n\n{title}\n{subtitle}",
        ...
        card.grid(row=0, column=column, padx=25, pady=20)
    ...
    def toggle_dark_mode(self):
        ...
        if self.dark_mode:
            ...
            self.dark_btn.configure(text="☀️ الوضع الفاتح")
        else:
            ctk.set_appearance_mode("light")
            self.dark_btn.configure(text="🌙 الوضع الداكن")
