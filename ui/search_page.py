...
class SearchPage(ctk.CTkFrame):
    ...
    def template_result(self, parent, row):
        ...
        ctk.CTkLabel(
            body,
            text=f"القسم: {category_name}  |  النوع: {source}  |  آخر تعديل: {updated_at}",
            font=("Segoe UI", 13),
            text_color=MUTED
        ...
        if self.app:
            ctk.CTkButton(
                ...
                height=34,
                fg_color=BLUE,
                command=self.app.show_documents
    ...
    def archive_result(self, parent, row):
        ...
        ctk.CTkLabel(
            body,
            text=f"الزبون: {customer_name or '-'}  |  الهاتف: {phone or '-'}  |  التاريخ: {created_at}",
            font=("Segoe UI", 13),
            text_color=MUTED
        ).pack(anchor="e", pady=(4, 0))
        ...
        if word_path:
            ctk.CTkButton(
                ...
                height=32,
                fg_color=BLUE,
                command=lambda p=word_path: self.safe_open(p)
        ...
        if pdf_path:
            ctk.CTkButton(
                ...
                height=32,
                fg_color=GREEN,
                command=lambda p=pdf_path: self.safe_open(p)
            ).pack(side="right", padx=3)
    ...
    def service_result(self, parent, row):
        ...
        ctk.CTkLabel(
            body,
            text=f"التاريخ: {created_at}  |  الرابط: {service_url or '-'}",
            font=("Segoe UI", 13),
            text_color=MUTED
        ).pack(anchor="e", pady=(4, 0))
        ...
        if service_url:
            ctk.CTkButton(
                ...
                height=34,
                fg_color=BLUE,
                command=lambda p=service_url: self.safe_open_url(p)
            ).pack(side="left", padx=14, pady=12)
    ...
    def customer_result(self, parent, row):
        ...
        ctk.CTkLabel(
            body,
            text=f"الهاتف: {phone or '-'}  |  العنوان: {address or '-'}",
            font=("Segoe UI", 13),
            text_color=MUTED
        ).pack(anchor="e", pady=(4, 0))
